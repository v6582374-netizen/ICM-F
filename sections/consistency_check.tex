\section{一致性检验与演化方程的实例化：软件开发人员的 $(\alpha,\beta,\mu_{\mathrm{occ}})$}

在通过"权威评估 $\rightarrow$ 能力维度 $\rightarrow$ 语义匹配 $\rightarrow$ 离散层级 $(s,c)$"对任务影响进行重标记后，
我们将任务DNA聚合回职业级冲击参数 $(\alpha,\beta,\mu_{\mathrm{occ}})$ 并首先进行一致性检验，
防止下游动力学依赖于错误实现的净冲击项。
...
\begin{equation}
    \mu_{\mathrm{occ}} = 0.629112-0.379113 = 0.249999.
\end{equation}
然而，预览JSON报告 $\mu_{\mathrm{occ}}=0.504112$，这与模型定义相矛盾。
因此，在将 $\mu_{\mathrm{occ}}$ 用作 $E(t)$ 的输入之前，聚合输出必须明确声明 \texttt{mu\_definition\_occ} 并修正实现；
否则，演化推断将系统性地产生偏差。
...
\subsection{修正 $\mu_{\mathrm{occ}}$ 后进入职业演化}

一旦 $\mu_{\mathrm{occ}}$ 被修正，演化方程变为
\begin{equation}
    \frac{dE(t)}{dt}=E(t)\big(g + A(t)\mu_{\mathrm{occ}}\big),
\end{equation}
其中 $A(t)$ 是采纳情景（快速/中速/慢速），$g$ 是非AI基线（公共趋势估计或在相对指数约定下设为 $0$）。
相对尺度具有闭式解
\begin{equation}
    \frac{E(t)}{E(t_0)}=\exp\left(\int_{t_0}^{t}\big(g+A(\tau)\mu_{\mathrm{occ}}\big)\,d\tau\right).
\end{equation}
因此，在 $\mu_{\mathrm{occ}}$ 定义一致且 $A(t)$/$g$ 明确指定之前，我们不推进到 $E(t)$ 情景曲线，避免不可计算的叙述。
