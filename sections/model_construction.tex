...
首先，我们需要将三个职业划定在三种特征维度上，以此为基础拉开三者的机制对比度，正好题目本身确定的三个领域就具备这样的条件，于是我们参考得出三个职业的特征参数为 $\textit{digit}(x)$、$\textit{physical}(x)$ 和 $\textit{iprisk}(x)$ ，然后我们需要针对美国所有的职业在这三个维度上进行打分，为了避免主观赋值，我们采用美国劳工部维护的\textbf{O*NET}职业数据库\cite{onet2025database}作为统一数据源。
...
为便于复现与审计，我们将上述6个描述符的访问直链与描述符ID集中维护在正文中（表~\ref{tab:onet_descriptor_urls}）。

\begin{table}[H]
\centering
\caption{O*NET描述符访问链接与描述符ID（用于复现）}
...
\toprule
\textbf{特征轴} & \textbf{描述符名称} & \textbf{描述符ID} & \textbf{URL} \\
\midrule
...
如图~\ref{fig:heterogeneity3d} 所示，三维散点图直观展示了最终优化结果。为了体现职业之间的两两之间的异质性，我们进一步绘制了该“职业三角”的二维投影，见图~\ref{fig:heterogeneity2d}。
...
由于官方给出的职业的原始任务描述字段太多，因此需要进行筛选。通过构建 Task DNA 的任务权重 $w_{ij}$，结合权威数据来源，在任务（而非职业）层面，计算其替代/增强强度 $(s_{ij},c_{ij})$，得到该职业的任务级净冲击为
\begin{equation}
    \mu_{ij}^{task}=c_{ij}-s_{ij},
\end{equation}
并将“当前轨迹”定义为社会的AI采用率 $A(t)\in[0,1]$，用于驱动任务需求随时间的演化。

进一步，我们开始构建任务层预测的数学模型。
...
基于公式~\eqref{eq:task_weight_norm}，我们通过Python编程计算各职业的任务权重（若所选SOC（标准职业分类）代码缺少任务频率量表 $FR_j$，则仅使用重要性值，即令 $\tilde{w}_j = IM_j$），进而选取三个职业的Top Task（20个以内），并将其可视化。分布结果见图~\ref{fig:task_dna_imfr_triptych}，任务文本图例见附录~\ref{app:task_dna_figures}。
...
结合三类职业的Task DNA，进一步给出针对性的解释：对于\textbf{机器人工程师}，其职业价值主要由少数高权重技术任务驱动；对于\textbf{一线主管（机械/安装/维修）}，点更分散且中等权重任务占比更高，反映管理、协调、现场判断等任务并存，结构变化由多项任务共同贡献；而\textbf{法庭速录与字幕}高度依赖少数语言处理核心任务。

得到三个职业的Top Task后，我们需要将其与当前Gen-AI能力进行映射，进而研究每一个Task受到的影响强度。那么我们同样需要对当今Gen-AI的能力进行解剖，划分出多个能力维度。以下的三点为主要的划分依据：
...
上表只展示了Gen-AI的九个维度，而
图~\ref{fig:genai_taskdna_chord_robotics}为各个维度映射到职业任务构成的更直观展示。
\begin{figure}[H]
\centering
...
继续基于上表得到的$s,c$数据，绘制\textbf{ $s,c$ 档位分布图}。该图的作用是\emph{揭示GenAI对该职业的潜在冲击模式}：若多数任务落在高 $s$ 低 $c$ 区域，表明该职业面临较高的自动化替代风险；反之，若多数任务落在低 $s$ 高 $c$ 区域，则GenAI更可能增强而非替代人类工作。
...
\begin{minipage}{0.32\linewidth}
    ...
    \small (c) 27-3092.00 法庭速录
\end{minipage}
\caption{三职业的权威 $s,c$ 档位分布对比}
\label{fig:task_dna_sc_triptych}
\end{figure}

结合图~\ref{fig:task_dna_sc_triptych}，我们从 $(s_{ij},c_{ij})$ 的结构分布解释三类职业的冲击机制，并与预测阶段的结果相互印证。对\textbf{机器人工程师}，针对机器人工程师主要关注右侧两条高s和高c柱、说明GenAI对其带来的净冲击较高，因此该职业需要更大程度的通过结构调整来适应发展。针对\textbf{一线主管（机械/安装/维修）}，最右侧柱高几乎为0，并且较多任务平均分散在中等高度的柱面，再次反应其任务的多元，且每个任务对总体结构的变化较平均。针对\textbf{法庭速录与字幕}，特征非常明显，最右侧只有一个高s柱，相应的c柱高则为0,再次说明其职业高度依赖少数核心任务。



得到$(s_{ij},c_{ij})$ 数据之后，便可计算$\mu^{task}_{ij}$，$\mu^{task}_{ij}>0$ 表示GenAI更可能增强该任务（提高产出/质量/速度），$\mu^{task}_{ij}<0$ 表示该任务更可能被替代或显著压缩。便可以进一步汇总三职业中负净冲击任务（$\mu^{task}_{ij}<0$）的权重与典型任务，结果见表~\ref{tab:task_sc_negative_impact} 
负净冲击（被替代倾向）
...
\subsubsection{将“发展轨迹”与“任务需求”紧密耦合}

题目要求“given the current trajectory and expected impacts of Gen-AI”，其中“current trajectory”我们将其定义为外生的采纳路径 $A(t)\in[0,1]$ 表示在时间 $t$ ，Gen-AI的采用率。并且，0被视为当前时间点，意味着从当前到未来的跨度。为确保该外生轨迹的数量级与现实一致，我们参考组织层面的AI采纳调查与行业预测所给出的趋势边界与情景上界~\cite{mckinsey2024stateai,mckinsey2025stateai,gartner2023genai}。

考虑到模型的适用性以及准确性，我们不对未来的任何时间点进行任何的定量预测，转而从更本质的角度出发，探讨未来任务相对份额的变化。我们通过引入几个指标来判定“任务的本质发生了变化”。
\paragraph{(指标i) 结构变化贡献率}
首先定义未归一化任务权重
...
按 $r_{ij}$ 降序排列，可识别出对结构变化贡献最大的任务（Top contributors）。
\paragraph{(指标ii) Top-$m$ 换血率}
...
\end{equation}
其中 $N_i$ 为职业 $i$ 的任务总数。$\widetilde I_i$ 作为 Kendall $\tau$ 距离的归一化形式\cite{kendall1938rank}，解释了任务相对重要性的结构变化称程度。
以上指标可作为"职业已重新定义"的主要判据：
\begin{equation}
...
阈值 $\theta_{I}$、$\theta_{J}$ 不作为固定常数，我们在多个阈值设置下计算结果，最后展示最优结果。

基于以上定义的三个指标，我们可以构建\textbf{排名翻转图}（图~\ref{fig:rank_flip_stem}）、\textbf{贡献分解条形图}（图~\ref{fig:rank_flip_stem} 右）两个核心可视化图标：

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig_STEM_rankflip_contribution_v2.pdf}
\caption{左图为：STEM职业的排名翻转。连线两端分别为 $\mathrm{rank}_i(j,t_0)$ 与 $\mathrm{rank}_i(j,t_1)$，翻转幅度越大，结构性重排越显著。右图为：STEM职业结构变化的 Top contributors 贡献分解。按 $r_{ij}$ 降序展示各任务对总份额变化的累积贡献，揭示结构重排的主要驱动任务。}
\label{fig:rank_flip_stem}
\end{figure}

正文中只展示机器人工程师的可视化图标，其余两个职业均采用同样的建模方式得出数据结果，并在章节3.2.3中给出详细分析。
\subsubsection{探索所选三个职业的未来}

基于以上任务预测模型、GenAI能力映射（表~\ref{tab:genai_scale}）以及上文定义的结构性变化指标，我们现在综合分析结果，探讨各职业在GenAI采用路径下的未来轨迹。

我们的分析表明，机器人工程师呈现出平衡但偏向增强的影响特征。图~\ref{fig:task_dna_sc_triptych}a显示，互补性得分 $c$ 集中在中高档次，而约 40\% 的任务权重处于负净影响区域，主要集中在维度 D3（算法编程与自动化），其中 $\mu = -0.20$。然而，这一部分的替代压力被工程问题解决（D1）和数据分析（D2）的强增强作用所抵消。这种” 此消彼长” 的对称模式表明：GenAI 并非单纯地” 消灭” 某类任务，而是触发
了职业内部的任务权重再平衡
...
我们的结论是，未来负责编排 AI 辅助的开发流程、验证机器生成的解决方案，并专注于需要物理世界判断和创造性问题解决将成为机器人工程师的主要任务——这些是当前 GenAI 无法复制的。

针对机械、安装与维修一线主管，其在三个职业中表现出最低的替代风险。$(s,c)$ 分布（图~\ref{fig:task_dna_sc_triptych}b）显著分散，没有单一维度主导影响特征。仅有一项任务（调度与派工，task\_id = 2912）具有负净影响，仅贡献0.059的总体任务权重。主导维度——D4（现场感知与物理操作）、D6（协作与培训）以及 D7（监督与质量控制）——均表现出正值或中性的 $\rho_k$ 值，表明 GenAI 的互补效应而非替代效应。
