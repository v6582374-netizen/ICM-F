...
\end{equation}
\indent\textbf{参数含义（通俗语言）。} $\kappa_s$ 是\emph{替代速度系数}。更大的 $\kappa_s$ 意味着在相同的 $A(t)$ 和 $s_{ij}$ 下人力努力下降更快。
...
\end{equation}
\indent\textbf{解释。} 高度自动化的任务（小的 $\ell_{ij}(t)$）失去份额；剩余的人力劳动转移到更难替代的任务。
...
\end{equation}
\indent\textbf{参数含义（通俗语言）。} $g_i$ 是功能需求的基线（非GenAI）增长；如果想避免宏观假设，可设 $g_i=0$。$\kappa_c\ge 0$ 是\emph{增强对需求}的放大因子。$\bar c_i$ 总结了职业在基线组成下的增强潜力。（一种变体用 $w_{ij}(t)$ 替代 $w_{ij}(0)$；我们使用 \eqref{eq:cbar} 以简化和参数稳定性。）
...
\end{equation}
\indent\textbf{解释。} 即使整体需求 $Q_i(t)$ 增长，如果任务份额 $w_{ij}(t)$ 因替代而崩溃，任务的人力需求仍可能缩减。
...
\end{equation}
\indent\textbf{参数含义（通俗语言）。} $k$ 控制陡峭程度（采纳加速的速度）。$t_0$ 是拐点时间，此时采纳达到50\%。我们通过选择三对 $(k,t_0)$ 定义三种情景（快速/中速/慢速）；这保持了驱动因子数量较少，同时支持敏感性比较。

\subsubsection{职业重新定义触发器（工作本质变化）}
\label{sec:redefinition}
我们通过\emph{任务消亡}和\emph{任务结构偏移}形式化"工作本质变化"。两个标准中任一标准均可触发重新定义。
...
\paragraph{(b) 任务结构偏移（分布变化）。}
即使没有许多任务完全变得可忽略不计，如果职业的任务份额分布发生足够大的变化，该职业也可能在性质上变得不同。令 $w_i(t)$ 表示向量 $(w_{i1}(t),\dots,w_{iJ_i}(t))$。我们使用Jensen--Shannon散度作为对称、有界的偏移度量：
\begin{equation}
\label{eq:JS_trigger}
...
$\delta$ & 例如 $0.10$--$0.30$ & 任务份额偏移阈值（JS散度） \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{任务结构变化：} $w_{ij}(t)$ 的可视化以及偏移程度 $\mathrm{JS}(w_i(t)\|w_i(0))$。
